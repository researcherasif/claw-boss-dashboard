-- Complete Migration Cache for Clawee Business Management
-- This file contains all database schema changes and can be run as a complete migration

-- Drop existing tables if they exist (for clean setup)
DROP TABLE IF EXISTS invoices CASCADE;
DROP TABLE IF EXISTS payment_calculations CASCADE;
DROP TABLE IF EXISTS pay_to_clowee CASCADE;
DROP TABLE IF EXISTS machine_settings_history CASCADE;
DROP TABLE IF EXISTS doll_price_history CASCADE;
DROP TABLE IF EXISTS machine_change_logs CASCADE;
DROP TABLE IF EXISTS machine_counter_reports CASCADE;
...
-- 2. Machines table (NO initial count columns)
...
-- 3. Machine Counter Reports table (WITH notes column for initial setup tracking)
CREATE TABLE machine_counter_reports (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE machine_change_logs (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    machine_id UUID NOT NULL REFERENCES machines(id) ON DELETE CASCADE,
    field TEXT NOT NULL,
    old_value TEXT,
...
CREATE TABLE doll_price_history (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    machine_id UUID NOT NULL REFERENCES machines(id) ON DELETE CASCADE,
...
CREATE TABLE machine_settings_history (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    machine_id UUID NOT NULL REFERENCES machines(id) ON DELETE CASCADE,
...
CREATE INDEX idx_machine_counter_reports_machine_id ON machine_counter_reports(machine_id);
CREATE INDEX idx_machine_counter_reports_date ON machine_counter_reports(report_date);
CREATE INDEX idx_machine_change_logs_machine_id ON machine_change_logs(machine_id);
CREATE INDEX idx_doll_price_history_machine_id ON doll_price_history(machine_id);
CREATE INDEX idx_machine_settings_history_machine_id ON machine_settings_history(machine_id);
CREATE INDEX idx_pay_to_clowee_machine_id ON pay_to_clowee(machine_id);
CREATE INDEX idx_payment_calculations_machine_id ON payment_calculations(machine_id);
...
COMMENT ON TABLE machines IS 'Machine configuration without initial counter data - initial counts stored in machine_counter_reports';